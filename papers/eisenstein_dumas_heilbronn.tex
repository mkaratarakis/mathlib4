...
We extend their results in three directions. First, we replace the Eisenstein condition by the
...
slightly stronger than the one usually quoted.
\end{abstract}
...
weakens the condition $\gcd(p-1,n)=1$.
...
\]
The sequence $\varphi$ is nonnegative and nonincreasing. Indeed, from
$\binom{q}{k+1}(k+1)=\binom qk (q-k)$ we get
$\binom{q}{k+1}(k+1)\le\binom qk q\,(k+1)$, hence $\binom{q}{k+1}\le q\binom qk$, which is
$\varphi(k+1)\le\varphi(k)$ after dividing by $q^{k+1}$.
...
\eqref{eq:pair-condition} holds by Lemma~\ref{lem:frobenius}. With $c_2=\frac14$, $c_3=\frac8{27}$ and $c_5\ge\frac8{25}$, the right-hand side of
\eqref{eq:complement} is nondecreasing in each $c_q$, so it is at least the value at
$(\frac14,\frac8{27},\frac8{25})$, namely
\[
...
for which we quote the classical inequality of P\'olya and Vinogradov \cite{Polya,Vinogradov}: for
a non-principal primitive Dirichlet character $\chi$ modulo $M$ and any $N$,
\begin{equation}\label{eq:PV}
  \Bigl|\sum_{u<N}\chi(u)\Bigr|\ \le\ C\,M^{1/2}\log M
\end{equation}
with an absolute constant $C$.

\begin{theorem}\label{thm:gcd}
Let $n\ge2$ and let $m\ge1$ be coprime to $n$. Put $Y=(\log p)^{1/5}$ and
$\hat\varepsilon(p)=(1+\pi(Y))(3/4)^{\pi(Y)}$. There is an absolute constant $p_0$ such that for
every prime $p\ge p_0$ with
\[
  g:=\gcd(p-1,n)\ <\ \frac{p^{1/2}}{(\log p)^2}
\]
one has $\liminf_{X\to\infty}\delta_{p,m,n}(X)\ \ge\ 1-\hat\varepsilon(p)$; in particular
$\hat\varepsilon(p)\to0$ as $p\to\infty$.
\end{theorem}
...
at least $1-\hat\varepsilon(p)$. Fix $f\in\mathcal F^{*}$ and primes $q_1<q_2\le Y$ modulo which
$f$ has no root. By Theorem~\ref{thm:main-ram}, $f$ is irreducible and $p$ is totally ramified in
$K_f$, so by Lemma~\ref{lem:heilbronn-applies} it suffices to produce positive integers $u,v$ with
\begin{equation}\label{eq:target}
  p=uq_1+vq_2,\qquad q_1\nmid u,\qquad q_2\nmid v,\qquad uq_1 \text{ an } n\text{-th power residue mod } p .
\end{equation}

Let $\psi$ be a Dirichlet character modulo $p$ of order $g$. Since the character group modulo $p$
is cyclic of order $p-1$ and $g\mid n$, such a $\psi$ exists; its kernel has index $g$ and
contains all $n$-th powers, hence equals the group of $n$-th powers, which also has index
$g=\gcd(n,p-1)$ in $(\Z/p\Z)^{\times}$. Thus for $p\nmid y$ we have $\psi(y)=1$ if and only if $y$
is an $n$-th power residue modulo $p$.
...
Put $N=p/q_1$ and count the positive integers $u<N$ with
\begin{equation}\label{eq:three}
  \gcd(u,q_1)=1,\qquad u\equiv c \pmod{q_2^{2}},\qquad \psi(q_1u)=1 .
\end{equation}
The three conditions are detected respectively by
\[
  \varphi_0(u),\qquad
  \frac{1}{\phi(q_2^{2})}\sum_{\chi \bmod q_2^{2}}\chi(u)\overline{\chi}(c),\qquad
  \frac1g\sum_{k=1}^{g}\psi^k(q_1u),
\]
where $\varphi_0$ is the principal character modulo $q_1$. Multiplying out, the number of such $u$
equals
\[
  \frac{1}{\phi(q_2^{2})\,g}\sum_{\chi\bmod q_2^{2}}\ \sum_{k=1}^{g}\ \overline{\chi}(c)\,\psi^k(q_1)
  \sum_{u<N}\bigl(\varphi_0\,\chi\,\psi^k\bigr)(u).
\]
The moduli $q_1$, $q_2^2$ and $p$ are pairwise coprime, so the character $\varphi_0\chi\psi^k$
modulo $M=pq_1q_2^{2}$ is principal precisely when $\chi=\chi_0$ and $\psi^k=1$, that is when
$(\chi,k)=(\chi_0,g)$. That pair contributes
\[
  \frac{1}{\phi(q_2^{2})g}\,\#\{u<N:\gcd(u,q_1q_2)=1\}
  =\frac{N}{\phi(q_2^{2})g}\Bigl(1-\frac1{q_1}\Bigr)\Bigl(1-\frac1{q_2}\Bigr)+O(1)
  =\frac{N}{q_2^{2}\,g}\Bigl(1-\frac1{q_1}\Bigr)+O(1),
\]
using $\phi(q_2^2)=q_2(q_2-1)$. Every other pair contributes a non-principal character sum modulo
$M$. For a primitive character these are bounded by \eqref{eq:PV}; a non-principal character
$\chi'$ modulo $M$ induced by a primitive $\chi^{*}$ modulo $M^{*}\mid M$ satisfies
$\sum_{u<N}\chi'(u)=\sum_{d\mid \mathrm{rad}(M)}\mu(d)\chi^{*}(d)\sum_{w<N/d}\chi^{*}(w)$, so the
same bound holds after multiplying by $2^{\omega(M)}\le 8$. Since there are $\phi(q_2^{2})g$ pairs
in all, the total contribution of the non-principal pairs is $O(M^{1/2}\log M)$ with an absolute
implied constant. Hence the number of $u<N$ satisfying \eqref{eq:three} is
\begin{equation}\label{eq:count}
  \frac{N}{q_2^{2}\,g}\Bigl(1-\frac1{q_1}\Bigr)+O\bigl(M^{1/2}\log M\bigr),
  \qquad M=pq_1q_2^{2}.
\end{equation}
Using $q_1\ge2$, $N=p/q_1$ and $q_1,q_2\le Y$, the main term is at least $p/(2Y^{3}g)$, while
$M\le pY^{3}$. So \eqref{eq:count} is positive as soon as
\[
  g\ <\ \frac{p^{1/2}}{C'\,Y^{9/2}\log (pY^{3})}
\]
for a suitable absolute constant $C'$. With $Y=(\log p)^{1/5}$ the right-hand side is
$\bigl(C'+o(1)\bigr)^{-1}p^{1/2}(\log p)^{-19/10}$, which exceeds $p^{1/2}(\log p)^{-2}$ for all
large $p$. Therefore the hypothesis on $g$ produces a positive integer $u<N=p/q_1$ satisfying
\eqref{eq:three}. Then $v=(p-uq_1)/q_2$ is a positive integer with $q_2\nmid v$, while
$q_1\nmid u$ and $\psi(q_1u)=1$ give the remaining two conditions in \eqref{eq:target}. By
Lemma~\ref{lem:heilbronn-applies}, $K_f$ is not norm-Euclidean.
\end{proof}
...
avoids the issue; the only cost is that the modulus of the character sum grows from $pq_1q_2$ to
$pq_1q_2^{2}$, which is absorbed by taking $Y=(\log p)^{1/5}$ in place of $(\log p)^{1/4}$.
\end{remark}
...
\begin{proof}
For odd $n$ this is Corollary~\ref{cor:positive}. For even $n$, choose a prime $p\ge p_0$ with
$p\equiv-1\pmod{2n}$, which exists by Dirichlet's theorem; then $\gcd(p-1,n)=2$, which is smaller
than $p^{1/2}(\log p)^{-2}$ for $p$ large, so Theorem~\ref{thm:gcd} applies and gives a positive
lower density inside $\mathcal F_{p,m,n}$, a family of positive density by
