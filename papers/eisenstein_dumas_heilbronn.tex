...
Let $\psi$ be a Dirichlet character modulo $p$ of order $g$. Such a $\psi$ exists because the
character group modulo $p$ is cyclic of order $p-1$ and $g=\gcd(p-1,n)$ divides $p-1$. Its kernel
has index $g$, and it contains every $n$-th power, since $\psi(y^n)=\psi(y)^n=1$ as $g\mid n$;
the $n$-th powers also form a subgroup of index $g$ in $(\Z/p\Z)^{\times}$, so the two subgroups
