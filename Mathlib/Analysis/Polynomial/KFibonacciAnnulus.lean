...
`LucasU.solution_ext` and `LucasU.unique` show sequences satisfying the hypotheses agree, so
every statement below is interderivable with its analogue for any concrete definition;
conversely the hypothesis form applies directly to `Nat.fib`, `Polynomial.Chebyshev.U` and
friends with no glue.
...
private lemma range_succ_eq_insert_Icc (n : ℕ) : range (n + 1) = insert 0 (Icc 1 n) := by
  ext x
...
theorem unique {G : ℕ → R} (hG0 : G 0 = 0) (hG1 : G 1 = 1)
    (hGrec : ∀ n, G (n + 2) = p * G (n + 1) + q * G n) (n : ℕ) : F n = G n :=
  congrFun (solution_ext (M := R)
    (fun j => by rw [hrec j, smul_eq_mul, smul_eq_mul])
    (fun j => by rw [hGrec j, smul_eq_mul, smul_eq_mul])
    (hF0.trans hG0.symm) (hF1.trans hG1.symm)) n
...
/-- Variant of `LucasU.sum_choose_mul` summing over `Icc 1 n`: the `ℓ = 0` term vanishes
because `F 0 = 0`.  This is the form in which the identity provides a partition of unity
for the annulus bounds below. -/
theorem sum_Icc_choose_mul (m n : ℕ) :
    ∑ ℓ ∈ Icc 1 n, (n.choose ℓ : R) * (q * F m) ^ (n - ℓ) * F (m + 1) ^ ℓ * F ℓ
      = F ((m + 1) * n) := by
  rw [← sum_choose_mul hF0 hF1 hrec m n, range_succ_eq_insert_Icc, sum_insert (by simp)]
  simp [hF0]
...
private lemma U_shift_zero : U R (((0 : ℕ) : ℤ) - 1) = 0 := by simp

private lemma U_shift_one : U R (((1 : ℕ) : ℤ) - 1) = 1 := by simp

private lemma U_shift_rec (j : ℕ) : U R (((j + 2 : ℕ) : ℤ) - 1)
...
theorem sum_choose_mul_U (m n : ℕ) :
  ...
  have h := LucasU.sum_choose_mul (p := 2 * X) (q := -1)
    (F := fun j => U R ((j : ℤ) - 1)) (U_shift_zero R) (U_shift_one R) (U_shift_rec R) m n
...
theorem U_sub_one_dvd {a b : ℕ} (h : a ∣ b) :
  ...
  LucasU.dvd_of_dvd (p := 2 * X) (q := -1) (F := fun j => U R ((j : ℤ) - 1))
    (U_shift_zero R) (U_shift_one R) (U_shift_rec R) h
...
theorem lucas_pos_of_ne_zero (hP : 0 < P) (hQ : 0 ≤ Q) {j : ℕ} (hj : j ≠ 0) : 0 < F j := by
  obtain ⟨t, rfl⟩ : ∃ t, j = t + 1 := ⟨j - 1, by omega⟩
  exact lucas_pos hF0 hF1 hrec hP hQ t
...
variable {k : ℝ} (hF0 : F 0 = 0) (hF1 : F 1 = 1)
  (hrec : ∀ n, F (n + 2) = k * F (n + 1) + F n)
include hF0 hF1 hrec
...
theorem kFib_three : F 3 = k ^ 2 + 1 :=
  lucas_three (P := k) (Q := 1) hF0 hF1 fun j => by rw [hrec j]; ring
...
/-! ### The analytic weight lemmas, over any normed ring with multiplicative norm -/
...
theorem norm_le_of_isRoot_of_sum_weights {p : K[X]} {A : ℕ → ℝ} {R : ℝ} (hR : 0 ≤ R)
  ...
  have hn1 : 1 ≤ n := Nat.pos_of_ne_zero fun h => by simp [h] at hsum
  have hlc : 0 < ‖p.leadingCoeff‖ :=
    norm_pos_iff.2 (leadingCoeff_ne_zero.2 (ne_zero_of_natDegree_gt hn1))
  obtain ⟨j, hj, hjpos⟩ : ∃ j ∈ Icc 1 n, 0 < A j :=
    exists_lt_of_sum_lt (f := fun _ => (0 : ℝ)) (by simp [hsum])
  -- Split the leading term off the evaluation of `p` at `z`.
  have heval : p.leadingCoeff * z ^ n = -∑ i ∈ range n, p.coeff i * z ^ i := by
    have h := hz
    rw [IsRoot.def, eval_eq_sum_range, ← hn, range_add_one, sum_insert (by simp)] at h
    rw [← coeff_natDegree, ← hn, eq_neg_iff_add_eq_zero]
    exact h
  -- Reindex `range n` as `Icc 1 n` via `i ↦ n - i`.
  have hreindex : ∑ i ∈ range n, ‖p.coeff i‖ * ‖z‖ ^ i
      = ∑ ℓ ∈ Icc 1 n, ‖p.coeff (n - ℓ)‖ * ‖z‖ ^ (n - ℓ) :=
    sum_nbij' (fun i => n - i) (fun ℓ => n - ℓ)
      (fun i hi => by simp only [mem_range] at hi; simp only [mem_Icc]; omega)
      (fun ℓ hℓ => by simp only [mem_Icc] at hℓ; simp only [mem_range]; omega)
      (fun i hi => by simp only [mem_range] at hi; omega)
      (fun ℓ hℓ => by simp only [mem_Icc] at hℓ; omega)
      (fun i hi => by simp only [mem_range] at hi; rw [Nat.sub_sub_self hi.le])
  ...
  have hkey : ∑ ℓ ∈ Icc 1 n, ‖p.coeff (n - ℓ)‖ * ‖z‖ ^ (n - ℓ)
    ...
    have hlt : ∑ ℓ ∈ Icc 1 n, ‖p.coeff (n - ℓ)‖ * ‖z‖ ^ (n - ℓ)
      ...
      · have h1 := hcoeff j hj
        have h2 : R ^ j < ‖z‖ ^ j :=
          pow_lt_pow_left₀ hcon hR (Nat.one_le_iff_ne_zero.1 (mem_Icc.1 hj).1)
    ...
    have hrw :
      ...
      rw [mul_assoc, mul_assoc, ← pow_add, Nat.add_sub_cancel' (mem_Icc.1 hℓ).2]
    rw [hrw, ← sum_mul, hsum, one_mul]
  -- Contradiction.
  refine absurd ?_ (lt_irrefl (‖p.leadingCoeff‖ * ‖z‖ ^ n))
...
/-- The analytic core of the lower bound in Theorem 1.2: if the weights `A ℓ` sum to `1`
over `1 ≤ ℓ ≤ n` and `‖a ℓ‖ * r ^ ℓ ≤ A ℓ * ‖a 0‖` for all `1 ≤ ℓ ≤ n`, then every root of
`p` has norm at least `r`.  (Unlike the upper bound, no nonnegativity of the weights is
...
theorem le_norm_of_isRoot_of_sum_weights {p : K[X]} {A : ℕ → ℝ} {r : ℝ}
  ...
  have hlc : 0 < ‖p.leadingCoeff‖ :=
    norm_pos_iff.2 (leadingCoeff_ne_zero.2 (ne_zero_of_natDegree_gt hn1))
  -- If the constant coefficient vanishes, the hypothesis at `ℓ = n` is already
  -- contradictory.
  ...
  have heval : p.coeff 0 = -∑ ℓ ∈ Icc 1 n, p.coeff ℓ * z ^ ℓ := by
    have h := hz
    rw [IsRoot.def, eval_eq_sum_range, ← hn, range_succ_eq_insert_Icc,
      sum_insert (by simp)] at h
  ...
  have hbound : ‖p.coeff 0‖ < ∑ ℓ ∈ Icc 1 n, A ℓ * ‖p.coeff 0‖ := by
    ...
    · rcases eq_or_ne (p.coeff j) 0 with hcj | hcj
      ...
      · have h2 : ‖z‖ ^ j < r ^ j :=
          pow_lt_pow_left₀ hcon (norm_nonneg z) (Nat.one_le_iff_ne_zero.1 (mem_Icc.1 hj).1)
        have h1 := hcoeff j hj
...
/-! ### The master closed-form annulus, for an arbitrary partition of unity -/

variable {p : K[X]} {A : ℕ → ℝ}

/-- Master closed-form upper radius: for **any** weights positive on `1 ≤ ℓ ≤ n` and
summing to `1` there, every root satisfies
...
theorem norm_le_sup_of_isRoot_of_sum_weights
  ...
  set n := p.natDegree with hn
  have hlc : 0 < ‖p.leadingCoeff‖ :=
    norm_pos_iff.2 (leadingCoeff_ne_zero.2 (ne_zero_of_natDegree_gt hdeg))
  ...
  · intro ℓ hℓ
    ...
    have h := mul_le_of_rpow_le zero_le_one (hA ℓ hℓ) (norm_nonneg (p.coeff (n - ℓ)))
      hlc (Nat.one_le_iff_ne_zero.1 (mem_Icc.1 hℓ).1) hle
    rw [mul_one] at h
    rw [mul_assoc]
    exact h

/-- Master closed-form lower radius: for **any** weights positive on `1 ≤ ℓ ≤ n` and
summing to `1` there, and `a ℓ ≠ 0` on that range, every root satisfies
...
theorem inf_le_norm_of_isRoot_of_sum_weights
  ...
  have hr0 : 0 ≤ (Icc 1 n).inf' (Finset.nonempty_Icc.2 hdeg) (fun ℓ =>
    ...
    exact Real.rpow_nonneg (mul_nonneg (hA ℓ hℓ).le
      (div_nonneg (norm_nonneg _) (norm_nonneg _))) _
  refine le_norm_of_isRoot_of_sum_weights hsum ?_ hz
  intro ℓ hℓ
  ...
  have hpow : ((Icc 1 n).inf' (Finset.nonempty_Icc.2 hdeg) fun ℓ =>
    ...
    calc _ ≤ ((A ℓ * (‖p.coeff 0‖ / ‖p.coeff ℓ‖)) ^ ((ℓ : ℝ)⁻¹)) ^ ℓ :=
      ...
      _ = _ := Real.rpow_inv_natCast_pow hb0 (Nat.one_le_iff_ne_zero.1 (mem_Icc.1 hℓ).1)
...
section LucasAnnulus

/-! ### The Lucas-weight annulus -/

variable {p : K[X]} {P Q : ℝ} {F : ℕ → ℝ}
variable (hF0 : F 0 = 0) (hF1 : F 1 = 1) (hrec : ∀ j, F (j + 2) = P * F (j + 1) + Q * F j)
include hF0 hF1 hrec

private lemma N_nonneg (hP : 0 < P) (hQ : 0 ≤ Q) (n m ℓ : ℕ) :
    0 ≤ (n.choose ℓ : ℝ) * (Q * F m) ^ (n - ℓ) * F (m + 1) ^ ℓ * F ℓ :=
...
theorem norm_le_of_isRoot_lucas (hP : 0 < P) (hQ : 0 ≤ Q)
  ...
  set n := p.natDegree with hn
  have hD : 0 < F ((m + 1) * n) :=
    Real.lucas_pos_of_ne_zero hF0 hF1 hrec hP hQ (by positivity)
  refine norm_le_of_isRoot_of_sum_weights
    (A := fun ℓ =>
      (n.choose ℓ : ℝ) * (Q * F m) ^ (n - ℓ) * F (m + 1) ^ ℓ * F ℓ / F ((m + 1) * n))
    hR (fun ℓ _ => div_nonneg (N_nonneg hF0 hF1 hrec hP hQ n m ℓ) hD.le) ?_ ?_ hz
  · rw [← sum_div, LucasU.sum_Icc_choose_mul hF0 hF1 hrec m n, div_self hD.ne']
  · intro ℓ hℓ
...
theorem le_norm_of_isRoot_lucas (hP : 0 < P) (hQ : 0 ≤ Q)
...
theorem norm_le_lucas_sup_of_isRoot (hP : 0 < P) (hQ0 : 0 < Q)
  ...
  have hNpos : ∀ ℓ ∈ Icc 1 n,
    ...
    positivity
  refine norm_le_of_isRoot_lucas hF0 hF1 hrec hP hQ0.le (m' + 1) hdeg ?_ ?_ hz
  ...
  · intro ℓ hℓ
    ...
    exact mul_le_of_rpow_le (hFnn _) (hNpos ℓ hℓ) (norm_nonneg _) hlc
      (Nat.one_le_iff_ne_zero.1 (mem_Icc.1 hℓ).1) hle
...
theorem lucas_inf_le_norm_of_isRoot (hP : 0 < P) (hQ0 : 0 < Q)
  ...
  obtain ⟨m', rfl⟩ : ∃ m', m = m' + 1 := ⟨m - 1, by omega⟩
  have hD : 0 < F ((m' + 1 + 1) * n) :=
    Real.lucas_pos_of_ne_zero hF0 hF1 hrec hP hQ0.le (by positivity)
  have hr0 : 0 ≤ (Icc 1 n).inf' (Finset.nonempty_Icc.2 hdeg) (fun ℓ =>
      ((n.choose ℓ : ℝ) * (Q * F (m' + 1)) ^ (n - ℓ) * F (m' + 1 + 1) ^ ℓ * F ℓ
        / F ((m' + 1 + 1) * n) * (‖p.coeff 0‖ / ‖p.coeff ℓ‖)) ^ ((ℓ : ℝ)⁻¹)) := by
    refine Finset.le_inf' _ _ fun ℓ hℓ => ?_
    exact Real.rpow_nonneg (mul_nonneg
      (div_nonneg (N_nonneg hF0 hF1 hrec hP hQ0.le n (m' + 1) ℓ) hD.le)
      (div_nonneg (norm_nonneg _) (norm_nonneg _))) _
  refine le_norm_of_isRoot_lucas hF0 hF1 hrec hP hQ0.le (m' + 1) hdeg ?_ hz
  intro ℓ hℓ
  have hc : 0 < ‖p.coeff ℓ‖ := norm_pos_iff.2 (ha ℓ hℓ)
  exact mul_rpow_le_of_le hD (N_nonneg hF0 hF1 hrec hP hQ0.le n (m' + 1) ℓ)
    (norm_nonneg _) hc (Nat.one_le_iff_ne_zero.1 (mem_Icc.1 hℓ).1) hr0
    (Finset.inf'_le _ hℓ)
...
theorem isRoot_mem_lucas_annulus (hP : 0 < P) (hQ0 : 0 < Q)
  ...
  ⟨lucas_inf_le_norm_of_isRoot hF0 hF1 hrec hP hQ0 hm hdeg ha hz,
    norm_le_lucas_sup_of_isRoot hF0 hF1 hrec hP hQ0 hm hdeg hz⟩

end LucasAnnulus

section PaperAnnulus

/-! ### The theorems of the paper: `k`-Fibonacci and Fibonacci radii -/

variable {p : K[X]} {k : ℝ} {F : ℕ → ℝ}
variable (hF0 : F 0 = 0) (hF1 : F 1 = 1) (hrec : ∀ j, F (j + 2) = k * F (j + 1) + F j)
include hF0 hF1 hrec
    ...
    (Icc 1 p.natDegree).inf' (Finset.nonempty_Icc.2 hdeg) (fun ℓ =>
      ...
      ‖z‖ ≤ (Icc 1 p.natDegree).sup' (Finset.nonempty_Icc.2 hdeg) fun ℓ =>
        (F ((m + 1) * p.natDegree)
            / ((p.natDegree.choose ℓ : ℝ) * F m ^ (p.natDegree - ℓ) * F (m + 1) ^ ℓ * F ℓ)
            * (‖p.coeff (p.natDegree - ℓ)‖ / ‖p.leadingCoeff‖)) ^ ((ℓ : ℝ)⁻¹) := by
  have h := isRoot_mem_lucas_annulus (P := k) (Q := 1) hF0 hF1
    (fun j => by rw [hrec j]; ring) hk one_pos hm hdeg ha hz
  simpa using h
...
theorem isRoot_mem_kFib_annulus_four (hk : 0 < k) (hdeg : 1 ≤ p.natDegree)
    (ha : ∀ ℓ ∈ Icc 1 p.natDegree, p.coeff ℓ ≠ 0) {z : K} (hz : p.IsRoot z) :
    (Icc 1 p.natDegree).inf' (Finset.nonempty_Icc.2 hdeg) (fun ℓ =>
      ...
      ‖z‖ ≤ (Icc 1 p.natDegree).sup' (Finset.nonempty_Icc.2 hdeg) fun ℓ =>
        (F (4 * p.natDegree)
            / ((p.natDegree.choose ℓ : ℝ) * (k ^ 2 + 1) ^ (p.natDegree - ℓ)
                * (k ^ 3 + 2 * k) ^ ℓ * F ℓ)
            * (‖p.coeff (p.natDegree - ℓ)‖ / ‖p.leadingCoeff‖)) ^ ((ℓ : ℝ)⁻¹) := by
  have h := isRoot_mem_kFib_annulus hF0 hF1 hrec hk (m := 3) (by norm_num) hdeg ha hz
...
theorem isRoot_mem_fib_annulus {p : K[X]} (hdeg : 1 ≤ p.natDegree)
    (ha : ∀ ℓ ∈ Icc 1 p.natDegree, p.coeff ℓ ≠ 0) {z : K} (hz : p.IsRoot z) :
    (Icc 1 p.natDegree).inf' (Finset.nonempty_Icc.2 hdeg) (fun ℓ =>
  ...
  have h := isRoot_mem_kFib_annulus (k := 1) (F := fun j => (Nat.fib j : ℝ))
    (by exact_mod_cast Nat.fib_zero) (by exact_mod_cast Nat.fib_one)
    (fun j => by push_cast [Nat.fib_add_two]; ring) one_pos (m := 3) (by norm_num) hdeg
    ha hz
