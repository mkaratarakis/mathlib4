...
lemma c₁_neq_zero : h7.c₁ ≠ 0 := Ne.symm (Int.ne_of_lt h7.one_leq_c₁)

lemma one_leq_abs_c₁ : 1 ≤ |↑h7.c₁| := Int.one_le_abs (Ne.symm (Int.ne_of_lt h7.one_leq_c₁))
...
lemma c₁b (n : ℕ) : 1 ≤ n → (k : ℕ) → k ≤ n - 1 → (a : ℕ) → 1 ≤ a → (b : ℕ) → 1 ≤ b →
...
include hq0 h2mq in
lemma h0m : 0 < h7.m * h7.n q := mul_pos (h7.hm) (one_le_n h7 q hq0 h2mq)
...
lemma c₁IsInt (u : Fin (h7.m * h7.n q)) (t : Fin (q * q)) :
  ...
  rw [triple_comm h7.K
    ...
    (h7.γ' ^ (b q t * h7.l q u))]
  rw [mul_assoc]
  apply IsIntegral.mul _ (IsIntegral.mul (c1a h7 q hq0 u t) (c1c h7 q hq0 u t))
...
lemma sum_b (i1 i2 j1 j2 : ℕ) (Heq : ¬i2 = j2) : i1 + i2 • h7.β ≠ j1 + j2 • h7.β := by
    intros H
    apply (h7.hirr (i1 - j1) (j2 - i2))
    have h1 : i1 + i2 • h7.β = j1 + j2 • h7.β  ↔ (i1 + i2 • h7.β) - (j1 + j2 • h7.β) = 0 :=
      (sub_eq_zero (a := (i1 + i2 • h7.β : ℂ)) (b := (j1 + j2 • h7.β : ℂ))).symm
    rw [h1] at H
    have h2 : ↑i1 + ↑i2 • h7.β - (↑j1 + ↑j2 • h7.β) = 0 ↔
        ↑i1 + i2 • h7.β - ↑j1 - ↑j2 • h7.β = 0 := by
      simp_all only [nsmul_eq_mul, iff_true, sub_self, add_sub_cancel_left]
    rw [h2] at H
    have h3 : ↑i1 + i2 • h7.β - ↑j1 - j2 • h7.β = 0 ↔
          ↑i1 - ↑j1 + ↑i2 • h7.β - ↑j2 • h7.β = 0 := by ring_nf
    rw [h3] at H
    ...
    have h6 : ↑i1 - ↑j1 = - ((i2 - ↑j2 : ℂ) • h7.β) ↔ ↑i1 - ↑j1 = (↑j2 - ↑i2 : ℂ) • h7.β := by
      refine Eq.congr_right ?_
      simp only [smul_eq_mul]
      rw [← neg_mul, neg_sub]
    rw [h6] at H
    have h7 : (↑i1 - ↑j1 : ℂ) = (↑j2 - ↑i2 : ℂ) • h7.β ↔ (↑i1 - ↑j1) / (↑j2 - ↑i2 : ℂ) = h7.β := by
...
lemma sys_coe_ne_zero : h7.sys_coe q u t ≠ 0 := by
  unfold sys_coe
  rw [mul_assoc]
  refine mul_ne_zero (mod_cast β'_neq_zero h7 q t (h7.k q u)) ?_
  · exact mul_ne_zero (mod_cast α'_neq_zero h7 q u t) (mod_cast γ'_neq_zero h7 q u t)
...
lemma housec1_gt_zero : 0 ≤ @house.c₁ h7.K _ _ h7.hd := by
  apply mul_nonneg (by aesop) (by simp)
...
lemma c2_abs_val_pow : ↑|(h7.c₂ ^ h7.n q : ℤ)| ≤ (h7.c₂ ^ h7.n q : ℤ) := by
  simp only [abs_pow]
  refine (pow_le_pow_iff_left₀ (abs_nonneg _) (h7.zero_leq_c₂)
...
lemma c_coeffspow' :
  ((h7.c₁ : ℤ) ^ (h7.n q - 1) * (h7.c₁ : ℤ) ^ (h7.m * q) * (h7.c₁ : ℤ) ^ (h7.m * q)) =
    ((h7.c₁ : ℤ) ^ (h7.n q - 1 - h7.k q u) *
      ...
      ((h7.c₁ : ℤ) ^ h7.k q u * (h7.c₁ : ℤ) ^ (a q t * h7.l q u) *
        (h7.c₁ : ℤ) ^ (b q t * h7.l q u)) := by
  have hk : h7.k q u ≤ h7.n q - 1 :=
    Nat.le_pred_of_lt ((finProdFinEquiv.symm.toFun u).2.isLt)
  have hal : a q t * h7.l q u ≤ h7.m * q := by
    simpa [Nat.mul_comm] using h7.al_leq_mq (q := q) (u := u) (t := t)
  have hbl : b q t * h7.l q u ≤ h7.m * q := by
    simpa [Nat.mul_comm] using h7.bl_leq_mq (q := q) (u := u) (t := t)
  have hpowN :
      (h7.c₁ : ℤ) ^ (h7.n q - 1) =
        (h7.c₁ : ℤ) ^ (h7.n q - 1 - h7.k q u) * (h7.c₁ : ℤ) ^ h7.k q u := by
    simpa [Nat.sub_add_cancel hk] using
      (pow_add (h7.c₁ : ℤ) (h7.n q - 1 - h7.k q u) (h7.k q u))
  have hpowA :
      (h7.c₁ : ℤ) ^ (h7.m * q) =
        (h7.c₁ : ℤ) ^ (h7.m * q - a q t * h7.l q u) *
          (h7.c₁ : ℤ) ^ (a q t * h7.l q u) := by
    simpa [Nat.sub_add_cancel hal] using
      (pow_add (h7.c₁ : ℤ) (h7.m * q - a q t * h7.l q u) (a q t * h7.l q u))
  have hpowB :
      (h7.c₁ : ℤ) ^ (h7.m * q) =
        (h7.c₁ : ℤ) ^ (h7.m * q - b q t * h7.l q u) *
          (h7.c₁ : ℤ) ^ (b q t * h7.l q u) := by
    simpa [Nat.sub_add_cancel hbl] using
      (pow_add (h7.c₁ : ℤ) (h7.m * q - b q t * h7.l q u) (b q t * h7.l q u))
  rw [hpowN, hpowA]
  simp
  grind
...
include h2mq hq0 in
lemma foo : |↑q| ^ (h7.n q - 1) * ((1 + house h7.β') ^ (h7.n q - 1) *
   (house h7.α' ^ (h7.m * (2 * (h7.m * h7.n q))) * house h7.γ' ^ (h7.m * (2 * (h7.m * h7.n q))))) ≤
   (1 + house h7.β') ^ h7.n q * (√(2 * ↑h7.m) ^ h7.n q *
   (max 1 (house h7.α' ^ (2 * h7.m ^ 2) * house h7.γ' ^ (2 * h7.m ^ 2)) ^ h7.n q *
    √↑(h7.n q) ^ (↑(h7.n q : ℝ) - 1))) := by
...
lemma hAkl : --∀ (k : Fin (h7.m * h7.n q)) (l : Fin (q * q)),
  house ((algebraMap (𝓞 h7.K) h7.K) ((A h7 q) hq0 h2mq u t)) ≤
      ...
      · apply mul_le_mul
        · apply mul_le_mul
          ...
          · refine pow_le_pow_left₀ ?_ ?_ (h7.n q - 1)
            · apply house_nonneg
            · rw [← mul_assoc]; apply h7.house_add_mul_leq q t
          · apply pow_nonneg; apply house_nonneg
    ...
    · apply mul_le_mul
      · rw [← pow_add]; rw [← pow_add]; rw [← pow_add]
        ...
        · rw [add_mul, add_mul, one_mul]
          ...
          · simp only [tsub_le_iff_right]
            refine Nat.le_succ_of_le (Nat.le_add_right (h7.n q) (h7.k q u))
          · refine Nat.add_le_add (pow_c₂ h7 q u t h2mq) ?_
            · refine Nat.add_le_add (pow_c₂' h7 q u t h2mq) ?_
    ...
    · rw [h7.c₃_pow q]
      simp only [mul_assoc]
      apply mul_le_mul (by rfl) (foo h7 q hq0 h2mq) (by unfold house; positivity) ?_
    ...
    · unfold c₄
      ...
      · refine mul_le_mul_of_nonneg_right ?_ ?_
        · trans
          ...
          · nth_rw 1 [← Real.rpow_one (x := max 1 (house.c₁ h7.K * house.c₁ h7.K * 2 * ↑(h7.m)))]
            apply Real.rpow_le_rpow_of_exponent_le (le_max_left _ _)
              (mod_cast one_le_n h7 q hq0 h2mq)
        · simp only [Real.rpow_natCast]
          apply pow_nonneg (le_trans zero_le_one (one_leq_c₃ h7))
