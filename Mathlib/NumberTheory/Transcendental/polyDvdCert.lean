...
-- **Non-monic over a comm ring with a `IsUnit` hypothesis** — no field needed:
...
#print axioms big_dvd   -- axiom-free: no `ofReduceBool`, no `native_decide`
