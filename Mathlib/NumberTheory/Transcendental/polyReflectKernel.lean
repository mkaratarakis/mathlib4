...
lemma toPolyCore_insertK (i : ℕ) (c : R) (l : TL R) :
  ...
  | cons hd tl ih =>
    ...
    · simp only [hc, toPolyCore_cons, ih]; abel
...
lemma coeff_toPolyCore_listCoeff (D : ℕ) (l : TL R) :
  ...
  | cons hd tl ih =>
    ...
    · have h' : D ≠ i := Ne.symm h
      simp [listCoeff, List.filter_cons, h, h']
  ...
  | n + 1, q, p => by
    ...
    | (dq, cq) :: qt, (dp, cp) :: pt =>
      ...
      · -- a spurious zero leading term is dropped; `C 0 * X^dq = 0`, so `toPolyCore` is unchanged
        simp only [divModK, if_pos hz, toPolyCore_cons, hz, map_zero, zero_mul, zero_add]
