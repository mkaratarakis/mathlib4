...
-/
import Mathlib.NumberTheory.Transcendental.Sturm.JumpPoly
...
def cauchyIndex (p q : Polynomial R) (a b : R) : ℤ :=
  ∑ x ∈ rootsInInterval p a b, jump_val p q x

def variation (a b : R) : Int :=
  if a * b ≥ 0 then 0 else if a < b then 1 else -1

def cross (p : Polynomial R) (a b : R) : Int :=
  variation (p.eval a) (p.eval b)

lemma cross_no_root {a b: R} {p: Polynomial R} (hab: a < b) (hxnroot: rootsInInterval p a b = ∅) :
      (cross p a b = 0) := by
  have hone : ¬ (eval a p > 0 ∧ eval b p < 0) := by
    by_contra!
    have ⟨r, hr⟩ := intermediate_value_property_ioo (le_of_lt hab) (this.1) (this.2)
  ...
  have htwo: ¬ (eval a p < 0 ∧ eval b p > 0) := by
    by_contra!
    have ⟨r, hr⟩ := intermediate_value_property'_ioo (le_of_lt hab) (this.1) (this.2)
    have hr_int: r ∈ rootsInInterval p a b := by unfold rootsInInterval; aesop
...
lemma cauchyIndex_poly_mod (p q : Polynomial R) (a b : R) :
...
theorem variation_mult_pos1 (c x y : R) (hc : c > 0) : variation (c*x) y = variation x y := by
...
theorem variation_mult_pos2 (c x y : R) (hc : c > 0) : variation x (c*y) = variation x y := by
  rw[variation, variation]
  have sgnequals : (0 ≤ x*y) = (0 ≤ c*(x*y)) := by
    simp
    constructor
  ...
  have hneg : c * (x * y) < 0 → (x < c * y ↔ x < y) := by
    intro hcy
    simp_all
    have hxy : x * y < 0 := (lt_iff_lt_of_le_iff_le (mul_nonneg_iff_of_pos_left hc)).mp hcy
    have : (x < c * y) = (x < y) := by
      simp_all
      constructor
      · intro hcxley
...
theorem variation_mult_pos (c d x y : R) (hc : c > 0) (hd: d > 0): variation (c * x) (d*y) = variation x y := by
  simp_all [variation_mult_pos1, variation_mult_pos2]

lemma variation_cases (x y: R):
...
lemma variation_mult_neg_1 (c x y: R) (hc: c < 0): variation (c*x) y = variation x y + if y = 0 then 0 else sgn x := by
...
@[simp]
theorem cindex_poly_z_1 (p q: Polynomial R) (a b: R) (hp: p = 0) : cauchyIndex p q a b  = 0 := by
  simp [cauchyIndex, jump_val, hp]

@[simp]
theorem cindex_poly_z_2 (p q: Polynomial R) (a b: R) (hq: q = 0) : cauchyIndex p q a b  = 0 := by
  simp [cauchyIndex, jump_val, hq]

theorem cindex_poly_const (p q: Polynomial R) (a b x: R) (hp_const: C x = p): cauchyIndex p q a b = 0 := by
...
theorem cindex_poly_mult {p q p': Polynomial R} {a b: R} (hp' : p' ≠ 0) :
...
theorem cindex_poly_cross {p : Polynomial R} {a b: R} (hab: a < b) (hpa_nroot: eval a p ≠ 0) (hpb_nroot: eval b p ≠ 0) :
  ...
  | _ k ih  =>
    cases k with
    | zero =>
      have hp_const : ∃ x: R, C x = p := natDegree_eq_zero.mp hp
      have ⟨x, hx⟩ := hp_const
      ...
      | succ k =>
        if H: (rootsInInterval p a b).Nonempty then
          let maxr : R := Finset.max' (rootsInInterval p a b) H
          ...
          have ⟨p', hp'⟩ : ∃p': Polynomial R, p = p' * maxrp := by
            have : maxrp ∣ p := by
              unfold maxrp
          ...
          have hmonon_nvdv : ¬ (X - C maxr ∣ p') := by
            ...
            have : (X - C maxr)^(rootMultiplicity maxr p + 1) ∣ p := by
              unfold maxrp at hp';
              have ⟨p'', hp''⟩ : ∃ p'' : Polynomial R, p' = p'' * (X - C maxr) := exists_eq_mul_left_of_dvd this
                ...
                have hn: (eval maxr p' > 0) = (eval b p' > 0) := by
                  ...
                  have hprodz: (eval maxr p') * (eval b p') < 0 := by
                      ...
                      if H'': eval maxr p' > 0 then
                        ...
                        nlinarith
                  have ⟨r, hr, ⟨hrmaxr, hrp'⟩⟩ : ∃r: R, (r > maxr) ∧ ( r < b) ∧ (eval r p' = 0):= by
...
theorem cindex_poly_inverse_add {p q: Polynomial R} (a b: R) (hpq_coprime: IsCoprime p q) : cauchyIndex p q a b + cauchyIndex q p a b = cauchyIndex (q * p) 1 a b := by
...
theorem cindex_poly_inverse_add_cross (p q : Polynomial R) (a b : R)
...
lemma cindex_poly_congr (p q : Polynomial R) (a a' b b' : R) (haa' : a < a') (hb'b : b' < b) (ha'b' : a' < b')
    (hpx : ∀ x : R, ((a < x ∧ x ≤ a') ∨ (b' ≤ x ∧ x < b)) → eval x p ≠ 0) :
  ...
  have : rootsInInterval p a b = rootsInInterval p a' b' := by
    ...
    have : rootsInSet p (Set.Ioc a a') = ∅ := by
      by_contra!
      simp at this
      obtain ⟨x, hx⟩ : ∃ x : R, x ∈ {x ∈ p.roots.toFinset | a < x ∧ x ≤ a'} := Finset.nonempty_def.mp this
      simp at hx
    ...
    have : rootsInSet p (Set.Ico b' b) = ∅ := by
      by_contra!
      simp at this
      obtain ⟨x, hx⟩ : ∃ x : R, x ∈ {x ∈ p.roots.toFinset | b' ≤ x ∧ x < b} := Finset.nonempty_def.mp this
      simp at hx
      obtain ⟨⟨hx11, hx12⟩, hx2, hx3⟩ := hx
