...
-/
import Mathlib.NumberTheory.Transcendental.Sturm.SignRPos
...
-- 0 otherwise
def jump_val (p q : Polynomial R) (x : R) : ℤ :=
  ...
  have h_sign : sign_r_pos x (p' * q * (p' * p)) = sign_r_pos x (q * p) := by
    have ⟨b, h_b⟩ : ∃b, b > x ∧ (∀z, x < z ∧ z < b -> eval z (p' * p') > 0)  := by
      ...
      · have roots_fin : {r: R | eval r p' = 0 ∧ r > x}.Finite := by
        ...
        have h_eval_nz: (∀z: R, x < z ∧ z < lr -> eval z p' ≠ 0) ∧ lr > x := by
          have : lr > x := by
...
lemma jump_poly_smult_1 (p q: Polynomial R) (c x: R) :
...
@[simp]
lemma jump_poly_not_root {p q: Polynomial R} {x: R} (hp: eval x p ≠ 0) : jump_val p q x = 0 := by
  unfold jump_val
...
@[simp]
lemma jump_poly_z1 (p: Polynomial R) (x: R) : jump_val p 0 x = 0 := by simp [jump_val]

@[simp]
lemma jump_poly_z2 (q: Polynomial R) (x: R) : jump_val 0 q x = 0 := by simp [jump_val]

lemma jump_poly_coprime {p q: Polynomial R} {x: R} (hp: eval x p = 0) (hpq_coprime : IsCoprime p q) : jump_val p q x = jump_val (q*p) 1 x := by
...
lemma jump_poly_1_mult {p q: Polynomial R} {x: R} (hnroot: eval x p ≠ 0 ∨ eval x q ≠ 0) :
...
lemma jump_poly_sign (p q : Polynomial R) (x : R) :
