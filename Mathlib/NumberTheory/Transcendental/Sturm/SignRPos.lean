...
-/
import Mathlib.NumberTheory.Transcendental.Sturm.Utils
...
instance : TopologicalSpace R := Preorder.topology R
instance : OrderTopology R := by exact { topology_eq_generate_intervals := rfl }

def rightNear (x : R) : Filter R := nhdsWithin x (Set.Ioi x)

def eventually_at_right (x : R) (P : R → Prop) : Prop :=
  Filter.Eventually P (rightNear x)

theorem eventually_at_right_equiv {x : R} {P : R -> Prop} : eventually_at_right x P ↔ (∃ b : R, (b > x) ∧ (∀ y : R, x < y ∧ y < b → P y)) := by
  constructor
  · intro hev
...
lemma sign_r_pos_rec (p : Polynomial R) (x : R) (hp : p ≠ 0) :
  ...
  if hev : eval x p = 0 then
    ...
    have H2 : sign_r_pos x p → sign_r_pos x (derivative p) := by
      ...
      simp at H1
      have : ¬ (∀ z : R, x < z → z ≤ ub → eval z (derivative p) < 0) := by
        intro abs
    ...
    have : sign_r_pos x p → 0 < eval x p := by
      by_contra!
      obtain ⟨h1, h2⟩ := this
      obtain ⟨ub, hub1, hub2⟩  : ∃ ub : R, x < ub ∧ (∀ z : R, z > x ∧ z < ub → 0 < eval z p) := by
      ...
      obtain ⟨r, hr1, hr2, hr3⟩ := intermediate_value_property' p x ((ub + x) / 2) (by linarith) (le_of_lt H2) (le_of_lt H)
      have : r ≠ x := by
        intro abs
        rw [<- abs] at hev
        exact hev hr3
...
lemma sign_r_pos_minus (x : R) (p : Polynomial R) : p ≠ 0 → (sign_r_pos x p ↔ (¬ sign_r_pos x (-p))) := by
  intro hp
...
lemma sign_r_pos_mult (p q : Polynomial R) (x : R) (hp : p ≠ 0) (hq : q ≠ 0) :
    sign_r_pos x (p * q) = (sign_r_pos x p ↔ sign_r_pos x q) := by
  obtain ⟨ub, hub1, hub2⟩ : ∃ ub : R , x < ub ∧ ((∀ z ∈ Ioo x ub, 0 < eval z p) ∨ (∀ z ∈ Ioo x ub, eval z p < 0)) := by
    obtain ⟨ub, hub1, hub2⟩ := next_non_root_interval p x hp
    have := (not_eq_pos_or_neg_iff_1 p x ub).mp hub2
  ...
  obtain ⟨ub', hub1', hub2'⟩ : ∃ ub' : R , x < ub' ∧ ((∀ z ∈ Ioo x ub', 0 < eval z q) ∨ (∀ z ∈ Ioo x ub', eval z q < 0)) := by
...
lemma sign_r_pos_deriv (p : Polynomial R) (x : R) (hp : p ≠ 0) (hev : eval x p = 0) : sign_r_pos x (derivative p * p) := by
...
lemma sign_r_pos_mod {x : R} (p q: Polynomial R) (hp_eval: eval x p = 0) (hq_eval: eval x q ≠ 0) :
...
lemma sign_r_pos_smult (p: Polynomial R) (x c: R) : c ≠ 0 -> p ≠ 0 ->
  ...
  next hf =>
    ...
    have hy : ∀y: R, ((0 < eval y (C c * p)) = (0 < eval y (-p) )) := by
      simp_all
      intro y
      constructor
...
lemma sign_r_pos_power (a: R) (n: ℕ): sign_r_pos a ((X - C a)^n) := by
  induction n with
  | zero => rw [eventually_at_right_equiv']; simp; exact exists_gt a
  | succ n ih =>
    have hd: (derivative ((X - C a)^(n + 1)) = (C (n + 1: R)) *  (X - C a)^n) := by
    ...
    have hsrpos1: sign_r_pos  a ((X - C a)^ (n + 1)) = (sign_r_pos a ((C (n + 1: R)) *  ((X - C a)^n))) := by
      rw [sign_r_pos_rec ((X - C a)^ (n + 1)) a hnz, hd]
      simp [heval]
    have hsrpos2: sign_r_pos  a ((X - C a)^ (n + 1)) = sign_r_pos  a ((X - C a)^ n) := by
      rw [ne_eq] at hnz
      rw [hsrpos1, sign_r_pos_smult ((X - C a) ^ n) a (n + 1: R) (Nat.cast_add_one_ne_zero n) hnz']
      simp only [gt_iff_lt, ite_eq_left_iff, not_lt, eq_iff_iff, not_iff_self, imp_false, not_le]
      positivity
    rw [<- hsrpos2] at ih
    exact ih
