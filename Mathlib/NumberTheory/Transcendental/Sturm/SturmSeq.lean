...
-/
import Mathlib.NumberTheory.Transcendental.Sturm.CauchyIndex
...
def sturmSeq (f g : Polynomial R) : List (Polynomial R) :=
  if f = 0 then
    []
  else
    f::(sturmSeq g (-f%g))
    ...
    if g1: g = 0 then
      ...
      have : (-f % g).degree < g.degree := by
        ...
        have : g.natDegree = 0 → g ∣ f := by
          intro hg
          have : ∃ c : R, C c = g := natDegree_eq_zero.mp hg
...
def seqEval (k : R) : List (Polynomial R) → List R
| [] => []
| a::as => (eval k a)::(seqEval k as)

def seqEvalSgn (k : R) : List (Polynomial R) → List R
| [] => []
| a::as => (sgn (eval k a))::(seqEvalSgn k as)

def seqVar_ab (P: List (Polynomial R)) (a b: R): ℤ :=
  (seqVar (seqEval a P) : Int) - seqVar (seqEval b P)

def seqVarSturm_ab (p q: (Polynomial R)) (a b : R) : ℤ :=
  seqVar_ab (sturmSeq p q) a b

def seqVarAbove_a (P: List (Polynomial R)) (a : R) : ℤ :=
  (seqVar (seqEval a P) : Int) - seqVar (seq_sgn_pos_inf P)

def seqVarBelow_b (P: List (Polynomial R)) (b : R) : ℤ :=
  (seqVar (seq_sgn_neg_inf P) : Int) - seqVar (seqEval b P)

def seqVarR (P : List (Polynomial R)) : ℤ :=
  (seqVar (seq_sgn_neg_inf P) : Int) - seqVar (seq_sgn_pos_inf P)

def seqVarAboveSturm (p q : Polynomial R) (a : R) : ℤ :=
  seqVarAbove_a (sturmSeq p q) a

def seqVarBelowSturm (p q : Polynomial R) (b : R) : ℤ :=
  seqVarBelow_b (sturmSeq p q) b

def seqVarRSturm (p q : Polynomial R) : ℤ  :=
  seqVarR (sturmSeq p q)

lemma seqVarSgn : ∀ ps : List (Polynomial R), ∀ (k : R), seqVar (seqEval k ps) = seqVar (seqEvalSgn k ps)
...
lemma smod_nil_eq (p q : Polynomial R) :
    sturmSeq p q = [] ↔ p = 0 := by
  constructor
  · intro hs
    by_contra!
...
@[simp]
lemma smods_s_0_1 (p: Polynomial R) : sturmSeq 0 p = [] := (smod_nil_eq 0 p).mpr rfl

@[simp]
lemma smods_s_0_2 (p: Polynomial R) : sturmSeq p 0 = if p = 0 then [] else [p] := by
  split_ifs with H
  · exact (smod_nil_eq p 0).mpr H
  · unfold sturmSeq; simp [H]

@[simp]
theorem seqVarSturm_ab_z_1 (p: Polynomial R) (a b: R) : seqVarSturm_ab 0 p a b = 0 := by
  unfold seqVarSturm_ab seqVar_ab seqVar seqEval sturmSeq
  simp

@[simp]
theorem seqVarSturm_ab_z_2 (p: Polynomial R) (a b: R) : seqVarSturm_ab p 0 a b = 0 := by
  unfold seqVarSturm_ab seqVar_ab seqVar seqEval
  if H: p = 0 then simp [H]
  else simp [seqEval, H]

lemma cauchyIndex_poly_taq (p q : Polynomial R) (a b : R) :
...
theorem changes_itv_smods_rec {a b: R} {p q: Polynomial R} (hpqa: eval a (p * q)≠ 0) (hpqb: eval b (p * q) ≠ 0) :
  ...
  if H: p = 0 ∨ q = 0 ∨ p % q = 0 then
    ...
    · unfold seqVarSturm_ab seqVar_ab seqEval cross seqVar sturmSeq
     ...
     have ⟨ps, httl, htlmod⟩ : ∃ ps : List (Polynomial R), sturmSeq p q = p :: q :: -p%q:: ps ∧ sturmSeq q (-p%q) = q :: (-p%q) :: ps := by
...
theorem cauchyIndex_sturmSeq_aux (p q: Polynomial R) (a b: R) (hab: a < b): ∃ (a' b': R), a < a' ∧ a' < b' ∧ b' < b ∧ (∀p' ∈ sturmSeq p q, (∀ x: R, ((a < x ∧ x ≤ a') ∨ (b' ≤ x ∧ x < b)) -> eval x p' ≠ 0)) := by
  induction h: (sturmSeq p q) generalizing p q with
    | nil =>
      ...
      have ⟨haa', ha'b', hbb'⟩ : a < a' ∧ a' < b' ∧ b' < b := by
        ...
        · unfold a' b'; linarith
        · unfold b'; linarith
      have hn_root:(∀p' ∈ [], ∀ x:R, (((a < x ∧ x ≤ a') ∨ (b' ≤ x ∧ x < b)) → (eval x p' ≠ 0))) := by simp
      use a', b'
    | cons hd tl ih =>
      let r := - (p % q)
      have hpz: p ≠ 0 := by
      ...
      have ⟨a1, b1, haa1, ha1b1, hbb1, ha1b1_nroot⟩: ∃ (a1 b1: R), a < a1 ∧ a1 < b1 ∧ b1 < b ∧
           (∀p' ∈ tl, (∀ x: R, ((a < x ∧ x ≤ a1) ∨ (b1 ≤ x ∧ x < b)) -> eval x p' ≠ 0)) := by
        exact ih q r htl
      have ⟨a2, b2, haa2, ha2_nroot, hbb2, hb2_nroot⟩ : ∃ (a2 b2: R), a < a2 ∧ (∀x: R, (a < x ∧ x ≤ a2) -> eval x p ≠ 0) ∧
                                                         (b2 < b) ∧ (∀x: R, (b2 ≤ x ∧ x < b) -> eval x p ≠ 0) := by
      ...
      have ⟨haa', ha'b', hbb'⟩ : a < a' ∧ a' < b' ∧ b' < b := by
        unfold a' b'
        grind
      have h_rec: ∀p' ∈ tl, ∀x: R, ((a < x ∧ x ≤ a') ∨ (b' ≤ x ∧ x < b))  -> eval x p' ≠ 0 := by
      ...
      have h_final: ∀ x: R, ((a < x ∧ x ≤ a') ∨ (b' ≤ x ∧ x < b)) -> eval x p ≠ 0 := by
        unfold a' b'; intros x
...
lemma cauchyIndex_poly_rec (p q : Polynomial R) (a b: R) (hab : a < b)
...
lemma changes_itv_smods_congr (p q : Polynomial R) (a a' b b' : R) (hpa : eval a p ≠ 0) (hpb : eval b p ≠ 0)
    (haa' : a < a') (hb'b : b' < b)
    (no_root : ∀ p' ∈ sturmSeq p q, ∀ x : R, ((a < x ∧ x ≤ a') ∨ (b' ≤ x ∧ x < b)) → eval x p' ≠ 0) :
...
theorem cauchyIndex_sturmSeq (p q: Polynomial R) (a b : R) (hpa: p.eval a ≠ 0) (hpb : p.eval b ≠ 0) (hab : a < b) :
  ...
  | nil =>
   ...
   | cons hd tl ih =>
      ...
      if H: q = 0 then simp [H]
        ...
        let r := (-p % q)
        have ⟨ps, hps, hpsqr, htlps⟩: ∃ps : List (Polynomial R), sturmSeq p q = p :: q :: ps ∧ sturmSeq q r = q :: ps ∧ tl = q :: ps := by
