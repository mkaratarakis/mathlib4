...
theorem addCore_sorted : ∀ {x y : List (MvDegrees nvars × R)},
  ...
  · next i a x j b y =>
    ...
    · next ji =>
      ...
      · exact addCore_sorted hx' hy
    · -- In a `LinearOrder`, `¬(i < j)` and `¬(j < i)` give `i = j`.
      have eq_ij : i = j := by
      ...
      subst eq_ij
      dsimp only
      -- Handle the `if a + b = 0` generated by `addCore`.
      split
      · exact addCore_sorted hx' hy'
...
-- `dedupList` never increases the maximum degree of the list.
...
-- `dedupList` of a weakly-sorted list is strictly sorted.
omit [DecidableEq R] in
...
-- Mapping doesn't change degrees, so the list stays sorted.
omit [DecidableEq R] in
...
theorem coeff_toPolyCore_of_degLt {n : MvDegrees nvars}
  ...
  | cons hd tl ih =>
    ...
    rw [ih (fun x hx => h x (List.mem_cons_of_mem _ hx)), zero_add]
...
-- Permuting the list does not change the polynomial.
omit [DecidableEq R] in
...
-- Combining like terms (`dedupList`) preserves the polynomial via distributivity.
omit [DecidableEq R] in
