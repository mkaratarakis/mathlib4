...
-- `[DecidableEq R]` is needed for the *tactic* (kernel `decide` on term lists) and the runtime ops,
-- but several pure-`toPolyCore` lemmas don't reference it; silence those bookkeeping lints.
...
lemma coeff_toPolyCore_listCoeff (D : MvDegrees nvars) (l : TL R nvars) :
  ...
  | cons hd tl ih =>
    ...
    by_cases h : i = D
    · subst h; simp [listCoeff, List.filter_cons]
    · rw [if_neg (fun he => h (toFinsupp_inj.mp he))]
      simp [listCoeff, List.filter_cons, h]
...
lemma toPolyCore_mReduceK [Div R] : ∀ (n : ℕ) (p : TL R nvars) (gs : List (TL R nvars)),
  ...
  | 0, p, _ => by simp [mReduceK, toPolyCore]
  | n + 1, p, gs => by
    match p with
    | [] => simp [mReduceK, toPolyCore]
    | (dp, cp) :: pt =>
      by_cases hz : cp = 0
      · simp only [mReduceK, if_pos hz, toPolyCore_cons, hz, map_zero, zero_add]
