...
most the degree, this yields at once that the polynomial is irreducible (the field generated by a
root has degree at least `n`, hence exactly `n`) and that `p` is totally ramified there,
`𝔭 ^ n = (p)`.
...
* `NumberField.EisensteinDumas.le_finrank`: `n ≤ [K : ℚ]`, which for a root of a degree-`n`
  polynomial is the Eisenstein–Dumas irreducibility criterion.
