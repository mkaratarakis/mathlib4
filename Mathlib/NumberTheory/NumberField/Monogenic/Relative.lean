...
This requires the base `𝓞 K` to be a principal ideal ring (class number one); over a
general Dedekind base the assembly needs localisation and is left open. -/

section GlobalPackaging

omit [NumberField K] [NumberField K₁] in
/-- **Transfer of `𝓞 K`-adjoin membership through the fraction field.**  For `β : 𝓞 K₁`,
if the image of `β` in `K₁` lies in the `𝓞 K`-subalgebra generated by the image of `θ`,
then `β` already lies in `Algebra.adjoin (𝓞 K) {θ}`. -/
private theorem mem_adjoin_of_algebraMap_mem {β : 𝓞 K₁}
    (h : algebraMap (𝓞 K₁) K₁ β ∈
      Algebra.adjoin (𝓞 K) {algebraMap (𝓞 K₁) K₁ θ}) :
    β ∈ Algebra.adjoin (𝓞 K) {θ} := by
  have hmap : (Algebra.adjoin (𝓞 K) {θ}).map (IsScalarTower.toAlgHom (𝓞 K) (𝓞 K₁) K₁)
      = Algebra.adjoin (𝓞 K) {algebraMap (𝓞 K₁) K₁ θ} := by
  ...
  rwa [FaithfulSMul.algebraMap_injective (𝓞 K₁) K₁ hxeq] at hx

/-- **Relative monogenicity over a principal ideal base.**  Let `θ : 𝓞 K₁` generate `K₁`
over `K`, and suppose `𝓞 K` is a principal ideal ring.  If `𝓞 K[θ]` is `π`-saturated in
`𝓞 K₁` for *every* prime element `π` of `𝓞 K` — the hypothesis discharged at each `π` by
`mem_adjoin_of_algebraMap_mul_mem` — then `𝓞 K[θ] = 𝓞 K₁`.

The proof clears denominators with the discriminant (`discr_mul_isIntegral_mem_adjoin`):
every `β : 𝓞 K₁` satisfies `d • β ∈ 𝓞 K[θ]` for a fixed nonzero `d : 𝓞 K`, and the
prime factorisation of `d` is peeled off one prime at a time using the saturation
hypothesis. -/
theorem adjoin_eq_top_of_forall_prime_saturated [IsPrincipalIdealRing (𝓞 K)]
    (hgen : Algebra.adjoin K {algebraMap (𝓞 K₁) K₁ θ} = ⊤)
    (hsat : ∀ π : 𝓞 K, Prime π → ∀ y : 𝓞 K₁,
      algebraMap (𝓞 K) (𝓞 K₁) π * y ∈ Algebra.adjoin (𝓞 K) {θ} →
      y ∈ Algebra.adjoin (𝓞 K) {θ}) :
    Algebra.adjoin (𝓞 K) {θ} = ⊤ := by
  classical
  -- integrality of the generator, over `𝓞 K` and over `K`
  ...
  -- the discriminant is a nonzero element of `𝓞 K`
  ...
  have hd₀ne : d₀ ≠ 0 := by
    ...
    apply hdne
    rw [← hd₀, h0, map_zero]
  -- the discriminant clears denominators: `d₀ * β ∈ 𝓞 K[θ]` for every `β`
  have hclear : ∀ β : 𝓞 K₁,
  ...
  have hP : ∀ x : 𝓞 K, x ≠ 0 → ∀ β : 𝓞 K₁,
      algebraMap (𝓞 K) (𝓞 K₁) x * β ∈ Algebra.adjoin (𝓞 K) {θ} →
      β ∈ Algebra.adjoin (𝓞 K) {θ} := by
    intro x
    ...
    · intro u hu _ β hβ
      obtain ⟨v, rfl⟩ := hu
      have hkey : algebraMap (𝓞 K) (𝓞 K₁) (↑v⁻¹) *
          (algebraMap (𝓞 K) (𝓞 K₁) (↑v) * β) = β := by
